...
public class GridLayoutManager2 extends LinearLayoutManager {
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        while (count < mSpanCount && layoutState.hasMore(state) && remainingSpan > 0) {
            ...
            // If view is already in the current mSet, do not add to mSet.
            boolean shouldSkipView = false;
            for (View value : mSet) {
                if (view == value) {
                    shouldSkipView = true;
                    break;
            ...
            if (shouldSkipView) {
                // Skip adding to mSet. This avoids an issue where an item is not placed in the grid correctly
                break;
            }
        ...
        Log.d(TAG, Arrays.toString(mSet));
        for (int i = 0; i < count; i++) {
            View view = mSet[i];
            LayoutParams params = (LayoutParams) view.getLayoutParams();
            Log.d(TAG, "before laid out child at position " + getPosition(view) + ", with l:"
            ...
            if (DEBUG) {
                Log.d(TAG, "after laid out child at position " + getPosition(view) + ", with l:"
                        + (left + params.leftMargin) + ", t:" + (top + params.topMargin) + ", r:"
                        + (right - params.rightMargin) + ", b:" + (bottom - params.bottomMargin)
                        + ", span:" + params.mSpanIndex + ", spanSize:" + params.mSpanSize);
            }
            ...
            if (params.isItemRemoved() || params.isItemChanged()) {
                Log.d(TAG, "ignoring! position " + getPosition(view));
                result.mIgnoreConsumed = true;
            }
            result.mFocusable |= view.hasFocusable();
        }
        Log.d(TAG,
                "focus " + result.mFocusable +
                " mFinished " + result.mFinished +
                " mConsumed " + result.mConsumed +
                " mIgnoreConsumed " + result.mIgnoreConsumed);
    ...
    private void assignSpans(RecyclerView.Recycler recycler, RecyclerView.State state, int count,
        ...
        Log.d(TAG,
                "layingOutInPrimaryDirection: " + layingOutInPrimaryDirection +
                " start:" + start +
                " end: " + end +
                " diff: " + diff);
        span = 0;
        for (int i = start; i != end; i += diff) {
            ...
            Log.d(TAG, "assign span at position " + getPosition(view) + ", spanIndex: "+ params.mSpanIndex + " spanSize: " + params.mSpanSize);
        }
    }
    ...
    public abstract static class SpanSizeLookup {
        ...
        public int getSpanIndex(int position, int spanCount) {
            ...
            if (span + positionSpanSize <= spanCount) {
                Log.d(TAG, "position " + position + " spanIndex " + span);
                return span;
            }
            return 0;
        }
